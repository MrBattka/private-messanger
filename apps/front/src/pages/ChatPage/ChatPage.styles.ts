...
export const MessageList = styled.div`
  ...
  background: linear-gradient(
    to right,
    ${props => props.theme.colors.backgroundGradientStart} 0%,
    ${props => props.theme.colors.backgroundGradientEnd} 100%
  );
  min-height: 0;
  ...
  scrollbar-color: ${props => props.theme.colors.primary} ${props => props.theme.colors.chatBackground};
`;
...
export const PlugSelectChat = styled.p`
  ...
  font-size: 24px;
  letter-spacing: 1px;
  color: ${props => props.theme.colors.textSecretMessenger};
`;
...
export const ScrollToBottomButton = styled.button<ScrollToBottomButtonProps>`
  position: absolute;
  bottom: ${({ $replyActive }) => ($replyActive ? '130px' : '80px')};
  right: 20px;
  width: 40px;
  height: 40px;
  border-radius: 50%;
