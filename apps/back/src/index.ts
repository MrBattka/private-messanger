...
io.on('connection', (socket) => {
  ...
  socket.on('join_chat', async (chatId: number) => {
    ...
    console.log(`[Socket] Пользователь ${userId} присоединился к комнате user_${userId}`);
    try {
  ...
  socket.on('send_message', async (data: {
    ...
    try {
      ...
      const otherMembers = updatedChat!.members.filter(m => m.userId !== userId);
      const lastMessage = updatedChat!.messages[0];
        ...
        const unreadCount = !isSender
          ? await prisma.message.count({
            where: {
              ...
              createdAt: { gt: lastSeen || new Date(0) },
            },
          })
          : 0;

        let displayName = updatedChat!.name;
        if (updatedChat!.members.length === 2) {
          const otherUser = updatedChat!.members.find(m => m.userId !== member.userId);
          if (otherUser) displayName = otherUser.user.username;
        }

        // 📮 Отправляем обновление чата только этому пользователю
        io.to(`user_${member.userId}`).emit('update_chat', {
          id: updatedChat!.id,
          name: displayName,
          ...
          unreadCount,
          isOnline: true,
          participantId: updatedChat!.members.length === 2
            ? updatedChat!.members.find(m => m.userId !== member.userId)?.userId || null
            : null,
        });
      }
